...
const AllPieCharts = () => {
  ...
  const [inTransitStageTotalParcels, setInTransitStageTotalParcels] =
    useState(0);
    ...
    const fetchData = async () => {
      try {
        ...
        const initialStage = rawData
          ...
          .filter((stage) =>
            ['OrderPlaced', 'PendingPickup'].includes(stage.status)
        ...
        const inTransitStage = rawData
          .filter((item) => item.group === 'In Transit')
          .flatMap((item) => item.subStages) // Extract subStages
          .filter((stage) =>
            [
              'PickedUp',
  ...
  return (
    <div className="flex gap-3">
      <PieChartMUI
        title="Initial Stage"
        chartData={initialStageParcels}
        totalParcels={initialStageTotalParcels}
        statusParcelCount="6"
      />
      <PieChartMUI
        title="In Transit Stage"
        chartData={inTransitParcels}
        totalParcels={initialStageTotalParcels}
        statusParcelCount="6"
      />
    </div>

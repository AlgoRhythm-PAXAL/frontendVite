  ...
  switch (field) {
    ...
    case 'staffId': {
      ...
      if (trimmedValue.length > 20) {
        return 'Staff ID must not exceed 20 characters';
      }
      const staffIdPattern = /^(STF|STAFF)\d{3,6}$/;
      if (!staffIdPattern.test(trimmedValue.toUpperCase())) {
        return 'Invalid Staff ID format. Use format like STF001 or STAFF001';
      }
      return null;
    }

    case 'status': {
      if (!['active', 'inactive'].includes(trimmedValue.toLowerCase())) {
...
export const validateAllStaffFields = (staffData) => {
  const errors = {};
  
  // Only validate fields that have values (for update operations)
  Object.keys(staffData).forEach(field => {
    if (staffData[field] && staffData[field].trim() !== '') {
      const error = validateField(field, staffData[field]);
      if (error) {
        errors[field] = error;
      }
    }
...
export const formatStaffContact = (contact) => {
  if (!contact) return '';
...
export const formatStaffName = (name) => {
  if (!name) return '';
  return name.trim().replace(/\s+/g, ' ');
};

export const formatStaffEmail = (email) => {
  if (!email) return '';
  return email.toLowerCase().trim();
};

export const formatStaffNIC = (nic) => {
  if (!nic) return '';
  return nic.trim().toUpperCase();
};

export const formatStaffId = (staffId) => {
  if (!staffId) return '';

// Admin validation utilities for frontend
export const validateField = (field, value) => {
  ...
  switch (field) {
    ...
    case 'adminId': {
      ...
      const adminIdPattern = /^ADMIN\d{3}$/;
      if (!adminIdPattern.test(trimmedValue.toUpperCase())) {
        return 'Invalid admin ID format (should be ADMIN followed by 3 digits)';
      }
      return null;
    }
...
export const validateAllAdminFields = (adminData) => {
  const errors = {};
  ...
  Object.keys(adminData).forEach(field => {
    if (adminData[field] && adminData[field].trim() !== '') {
      const error = validateField(field, adminData[field]);
      if (error) {
        errors[field] = error;
      }
    }
...
export const formatAdminContact = (contact) => {
  if (!contact) return '';
...
export const formatAdminName = (name) => {
  if (!name) return '';
  return name.trim().replace(/\s+/g, ' ');
};

export const formatAdminEmail = (email) => {
  if (!email) return '';
  return email.toLowerCase().trim();
};

export const formatAdminNIC = (nic) => {
  if (!nic) return '';
  return nic.trim().toUpperCase();
};

export const formatAdminId = (adminId) => {
  if (!adminId) return '';
  return adminId.trim().toUpperCase();
};

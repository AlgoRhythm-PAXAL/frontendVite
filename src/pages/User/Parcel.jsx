...
import { FiPackage, FiTruck, FiCheckCircle, FiClock, FiMapPin, FiDollarSign, FiPlus } from "react-icons/fi";
  ...
  useEffect(() => {
    const fetchParcels = async () => {
      try {
        ...
        setError(err.response?.data?.message || "Failed to load parcels. Please try again.");
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-800">
                  {parcels.filter(p => p.status === "InTransit" || p.status === "PickedUp").length}
                </p>
              </div>
            </div>
          </div>
        ...
        {filteredParcels.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <h3 className="mt-2 text-lg font-medium text-gray-900">No parcels found</h3>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredParcels.map((parcel) => (
                    <tr key={parcel._id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            ...
                            <div className="text-sm text-gray-500">{parcel.itemType}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div>
                            <div className="text-sm font-medium text-gray-900">{parcel.trackingNo}</div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{parcel.receiverId.receiverFullname}</div>
                        <div className="text-sm text-gray-500">{parcel.receiverId.receiverDistrict}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex space-x-2">
                          <span className="flex items-center text-sm text-gray-500">
                            <FiTruck className="mr-1" /> {parcel.shipmentMethod}
                          </span>
                          <span className="flex items-center text-sm text-gray-500">
                            <FiDollarSign className="mr-1" /> {parcel.paymentMethod}
                          </span>
                        </div>
                        <div className="text-sm text-gray-500 mt-1">
                          <FiMapPin className="inline mr-1" /> {parcel.parcelSize}
                        </div>
                      </td>

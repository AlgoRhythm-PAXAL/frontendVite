...
const AdminLogin = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.post('http://localhost:8000/admin/login', formData, { withCredentials: true, headers: { 'Content-Type': 'application/json' } });
            ...
            <div className="flex flex-col items-center justify-center w-1/3 bg-white pb-8 px-16 rounded-3xl relative z-10">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col items-center justify-center gap-5 w-full">
                    <div className="flex flex-col items-center justify-center w-full  gap-3">
                        <FormField
                            label="Email :" type="text" name="email" value={formData.email} onChange={handleChange} placeholder="admin@gmail.com" required
                        />
                        <FormField
                            label="Password: " type="password" name="password" value={formData.password} onChange={handleChange} placeholder="••••••••" required
                        />
                    </div>
                    <button type="submit" disabled={isLoading} className="w-2/3 px-4 py-3 bg-Primary text-white rounded-xl hover:bg-primary-dark transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                        {isLoading ? (
                    ...
                    <Link to="/admin/forgotPassword" className="text-Primary hover:text-primary-dark transition-colors">Forgot Password?</Link>
                </form>
            </div>
